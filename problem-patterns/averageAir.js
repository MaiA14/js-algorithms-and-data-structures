...
// array where the average of the pair equals the target average. There may be more than one pair
...
function averagePair(arr, num) {
    ...
    while (start < end) {
        if ((arr[start] + arr[end]) / 2 > num) {
            end--;
        } else if ((arr[start] + arr[end]) / 2 < num) {
            start++;
        } else {
            return true;
        }
